...
import { SkewAdjustedTime } from './skewAdjustedTime'

export const Formatter = {
    MILLISECONDS_TO_SECONDS_DIVISOR: 1000.0,

    formatDate: (date: number) => {
        return SkewAdjustedTime.moment()
            .to(date * 1000, true)
    ...
    formatMstoSecs: (milliseconds: number): number => {
        return milliseconds / Formatter.MILLISECONDS_TO_SECONDS_DIVISOR;
    },
     ...
     */
    formatPlural: (quantity: number, name: string): string => {
//        return `${quantity.toLocaleString()} ${pluralize(name, quantity)}`;
        // AB todo: fix above line, getting type error during build
        return `${quantity.toLocaleString()}`;        
    },

    formatToFixedPrecision: (value: number | null, decimalPlaces: number): number | null => {
        if (!value) {
            return value;
        }
